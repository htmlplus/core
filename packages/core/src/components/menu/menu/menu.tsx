...
import {MenuTunnel, MenuTunnelState} from './menu.tunnel';
...
export class Menu {
  ...
  bind() {
    ...
    this.$parent.addEventListener(this.eventTrigger, ()=>{})
  }

  unbind() {
    this.$parent.removeEventListener(this.eventTrigger, ()=>{})
  }
  ...
  get state(): MenuTunnelState {
    return {
      onClick: (value: number) => value
    }
  }
  ...
  render() {
    return (
      <Host>
        <MenuTunnel.Provider state={this.state} scope={this}>
          <slot/>
        </MenuTunnel.Provider>
      </Host>
    );
